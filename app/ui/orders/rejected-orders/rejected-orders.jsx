...
function RejectedOrders({handleOrderDetails}) {
  ...
  return (
    ...
    <div className="flex flex-wrap justify-between">
      {loading ? <PageLoader /> : orders && orders.length > 0 ? (
        orders.map((order, index) => <OrderCard key={index} order={order} handleRefresh={handleRefresh} handleOrderDetails={handleOrderDetails} />)
      ) : (
        <NoDataFound />
      )}
    </div>
